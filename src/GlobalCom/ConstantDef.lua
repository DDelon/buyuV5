...
FishCD.MSG_HEADER_FLAG = {
    NONE = 0,           -- //!<发送数据不做任何处理
    PACKET = 0x1,       -- //!<当前为包文件（一个或者多个连续包），每个包都有个包标记@brief msg_Packet 当所有包首发完毕才能处理
    MASK = 0x2,         -- //!<检查掩码,@ref   msg_Header::byMask 将当前包的校验码，当前包也会重组数据
    ENCODE = 0x4,       -- //!<重新编码,使用映射表重新编码数据
    COMPRESS = 0x8,     -- //!<压缩数据,数据将启用zlib压缩，该标记仅推荐，如果数据压缩后更大，将忽略该标记
    ROUTE = 0x10,       -- //!<路由包,服务器每次中转，@ref    msg_Header::byRouteCount 的值累加1，当到达特定值后，仍然没有到达目标服务器，该包将丢弃
    DELAYSEND = 0x20,   -- //!<延迟消息，等待下次非延迟消息合并发送（客户端忽略该标记）
    FILLED = 0x40,      -- //!<已经填充头部（已经进行掩码、编码、压缩处理)，不能重复处理
    OFFSET = 0x80,      -- //!<指定消息头偏移，掩码、编码、压缩处理将从消息头开始偏移@ref msg_Header::byHeaderOffset 个字节开始
}
