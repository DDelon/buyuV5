...
static void writeDataLog(string typeName, string data, bool isSend) {

	
	if (isSend) {
		DebugLog::getInstance()->print("NetMsg", "-------- send --------");
	}
	else {
		DebugLog::getInstance()->print("NetMsg", "-------- recv --------");
	}
	
	DebugLog::getInstance()->print("NetMsg", "typeName="+typeName);
	DebugLog::getInstance()->print("NetMsg", "data=" + data+"\n");
}
...
static int lencodeBinary(lua_State* L) {
	JMsgWriter writer;
	...
	if (success) {
		...
		JMsgReader readerJson((unsigned char*)writer.getBuffer(), writer.getBufferLen());
		string json;
		proto->toJson(&readerJson, -1, json);
		writeDataLog(typeName, json, true);
...
static int lfreeBinary(lua_State* L) {
...
static int ldecodeBinary(lua_State* L) {
	JMsgProto* proto = (JMsgProto*)::lua_touserdata(L, -2);
	size_t len = 0;
	...
	lua_pop(L, 2);
	::lua_createtable(L, 0, 0);
	::JMsgReader reader((unsigned char*)ptr, 4096 * 2);
	...
	JMsgReader readerJson((unsigned char*)ptr, 4096 * 2);
	string json;
	proto->toJson(&readerJson, -1, json);

	if (msgType) {
		writeDataLog(msgType->m_typeName, json, false);
	}
...
void JMsgSetLogPath(std::string path) {
	DebugLog::getInstance()->setWritePath(path);
}
