...
* @brief 数据流序列化/反序列化 
*/
class DL_NOEXPORT CSerialStream
{
...
public:
	...
	*/
	CSerialStream(void* pData =NULL , size_t nSize = 0xFFFFffff);
	~CSerialStream(void);
	...
	* @param [in] nSize 数据流长度，如果为UINT_MAX则不检查长度
	*/
	void SetStreamData( void* pData = NULL, size_t nSize = 0xFFFFffff )
	{
	...
	*/
	bool WriteData(const void* pData,size_t nSize);
	...
	* @return 成功返回非0
	*/
	bool WriteStringW(const char16_t* lpWrite)
	{
		int nLen = 0;
		for(;lpWrite[nLen];++nLen)
			;
		return WriteData( lpWrite,sizeof(char16_t)*(nLen+1));
	}
	...
	* @return 成功返回非0
	*/
	bool ReadData(void* pData,size_t nSize);
...
protected:
	char*		m_pData;			//!<数据流地址
	size_t		m_nMaxSize;			//!<数据流长度
	size_t		m_nPosition;		//!<当前读写指针
};
